...
" " Version: 0.1
" "
" " To install, copy (or link) this file into the ~/.vim/syntax directory and
" add
...
"      
"      syn keyword kModule       module nextgroup=kModuleName skipwhite
"      endmodule
...
"      syn match   kSyntaxName   contained
"      "#\=\u\w*\({\s*#\=\u\w*\(\s*,\s*#\=\u\w*\)*\s*}\)*"
...
"      syn region  kCellContent  start="<\h\(\w\|-\)*>" end="</\h\(\w\|-\)*>"
"      fold transparent
"
"      syn match   kCell         /<\h\(\w\|-\)*\(\s\+\l\+="[^"]*"\)*>/
"      contains=kCellAttr,kString
"      syn match   kCell         "<\/\h\(\w\|-\)*>"
"      syn match   kCell         "\.\.\."
"
...
"      " syn keyword kAttrs        function predicate seqstrict strict hybrid
"      binder
...
"      syn match   kFloat
"      "\(\<\d\+\.\d*\|\.\d\+\)\([eE][-+]\=\d\+\)\=[fFdD]\="
"      syn region  kString     start=+"+ skip=+\\\\\|\\"+ end=+"+ end=+$+
"      contains=@Spell
"
